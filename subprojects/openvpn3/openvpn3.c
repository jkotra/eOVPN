...
_get_all_sessions ()
{
    ...
    error = NULL;
    GVariant *available_sessions =
        g_dbus_proxy_call_sync (sessions_proxy,
    ...
    while (g_variant_iter_next (iter, "o", &path))
        {
            ...
            if ((major == MAJOR_CONNECTION) &&
                (minor == MINOR_CONN_CONNECTED || MINOR_CONN_CONNECTING ||
                 MINOR_CONN_PAUSED || MINOR_CONN_PAUSING ||
                 MINOR_CONN_RECONNECTING || MINOR_CONN_RESUMING))
                {
                    error = NULL;
...
import_config (char *name, char *config_str)
{
    ...
    error = NULL;
    GVariant *result =
        g_dbus_proxy_call_sync (import_proxy,
...
{
    g_assert (UniqueSession != NULL);
    g_dbus_proxy_call (UniqueSession,
                       "Disconnect",
                       g_variant_new ("()"),
                       G_DBUS_PROXY_FLAGS_NONE,
                       -1,
